...
function LanguageSelector() {
  const { changeLanguage } = useLanguage(); // Obtener función para cambiar idioma del contexto
  const { t } = useTranslation();
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const toggleMenu = () => setIsMenuOpen(!isMenuOpen); // Alternar visibilidad del menú
  ...
  return (
    <div className="relative">
      ...
      >
        {/* SVG del icono personalizado */}
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          className="w-6 h-6 text-green-700 hover:text-green-900" // Ajusta el tamaño y color del ícono
          fill="currentColor"
        >
