...
export default function Home() {
    ...
    // Cargar notas y notas ocultas desde localStorage al montar el componente
    useEffect(() => {
        const savedNotes = JSON.parse(localStorage.getItem('notes')) || [];
        const savedHiddenNotes = JSON.parse(localStorage.getItem('hiddenNotes')) || [];
        setNotes(savedNotes);
        setHiddenNotes(savedHiddenNotes);
    }, []);

    // Guardar las notas ocultas en localStorage cada vez que cambien
    useEffect(() => {
    ...
    const handleHideNote = (index) => {
        const noteToHide = notes[index];
        setHiddenNotes((prevHidden) => [...prevHidden, noteToHide]);
    };

    const handleUnhideNote = (noteToUnhide) => {
        setHiddenNotes((prevHidden) => prevHidden.filter((note) => note !== noteToUnhide));
    };
    ...
    return (
        <div className="min-h-screen bg-gray-100">
            ...
            <div className="pt-20 p-4">
                <h1 suppressHydrationWarning className="text-4xl font-bold bg-gradient-to-r from-teal-400 to-teal-600 text-white p-6 rounded-lg shadow-lg text-center">
                    {t('welcome')} {user ? user : 'Invitado'}
                </h1>
                <p suppressHydrationWarning className="mt-4 text-teal-500 text-lg p-2 text-center italic">
                    {t('homedescription')}
                </p>
                <div className="mt-6 bg-white p-6 rounded-lg shadow-xl transition-shadow hover:shadow-2xl flex flex-col items-center">
                    <div className="flex flex-col w-full md:w-1/2 mb-4 items-center">
                        <h2 suppressHydrationWarning className="text-2xl font-semibold mb-2 text-teal-500 text-center">{t('feeling')}</h2>
                        <EmojiIcons selectedEmoji={selectedEmoji} onEmojiSelect={handleEmojiSelect} />
                        <Link href="/calendar">
                            <button suppressHydrationWarning className="bg-gradient-to-r from-emerald-300 to-emerald-500 text-white py-2 px-5 rounded-full hover:bg-emerald-600 transition duration-200 transform hover:scale-105 mt-2">
                                {t('days')}
                            </button>
                        </Link>
                    </div>
                    <div className="bg-emerald-200 px-4 py-2 rounded-lg mt-2 w-full shadow-md">
                        <h2 suppressHydrationWarning className="text-xl font-bold text-green-600 text-center">{t('notes')}</h2>
                        <div className="flex space-x-2 overflow-x-auto py-1 justify-center">
                            {visibleNotes.map((note, index) => (
                                <div key={index} className="bg-white shadow-lg rounded-lg p-4 w-64 flex-shrink-0 transition-transform transform hover:scale-105">
                                    <p suppressHydrationWarning className="text-sm">{note}</p>
                                    ...
                                    <button onClick={() => handleHideNote(index)} className="flex items-center mt-2 text-gray-500 hover:text-red-500 transition-colors duration-200">
                                        ...
                                        <span suppressHydrationWarning>{t('hide')} </span>
                                    </button>
                                </div>
                            ))}
                        </div>
                    ...
                    <Link href="/notas">
                        <button suppressHydrationWarning className="bg-gradient-to-r from-green-400 to-green-600 text-white py-2 px-5 rounded-full hover:bg-green-500 transition duration-200 transform hover:scale-105 mt-4">
                            {t('gonotes')}
                        </button>
                    </Link>
                </div>
            ...
            {isChatOpen && (
                <div className="fixed bottom-16 right-4 w-80 h-96 bg-white shadow-lg rounded-lg p-4 z-50">
                    <h2 suppressHydrationWarning className="text-lg font-bold">{t('help_you')}</h2>
