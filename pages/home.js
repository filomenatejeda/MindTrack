...
export default function Home() {
    ...
    const handleAddNote = (newNote) => {
        // Agregar nueva nota y respetar las notas ocultas
        const updatedNotes = [newNote, ...notes];
        const updatedVisibleNotes = updatedNotes.filter((note) => !hiddenNotes.includes(note)).slice(0, 3);
    ...
    const handleHideNote = (index) => {
        // Ocultar una nota visible
        const noteToHide = visibleNotes[index];
        const updatedHiddenNotes = [...hiddenNotes, noteToHide];
        setHiddenNotes(updatedHiddenNotes);

        // Guardar en localStorage
        localStorage.setItem('hiddenNotes', JSON.stringify(updatedHiddenNotes));
    };
    ...
    // Calcular notas visibles dinámicamente
    const visibleNotes = notes.filter((note) => !hiddenNotes.includes(note)).slice(0, 3);
