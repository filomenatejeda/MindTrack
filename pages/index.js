...
export default function Home() {
  ...
  <h2 className="text-3xl font-semibold text-teal-600 drop-shadow-lg">{t('features')}</h2>
  <div className="flex flex-wrap justify-center mt-8 gap-6"> {/* Cambiado de space-y-6 a gap-6 */}
    {['dailyemotion', 'exercises', 'habits', 'conextion'].map((feature, index) => (
      <div key={index} className="bg-white rounded-lg shadow-lg p-6 w-full sm:w-72 transition-transform transform hover:scale-105"> {/* Se agregó bg-white para evitar superposición */}
        <h3 className="text-xl font-bold text-green-600">{t(feature)}</h3>
        <p className="text-gray-600 mt-2">{t(`description${feature}`)}</p>
      </div>
    ))}
  </div>
</section>
        ...
        <div className="text-center mb-12">
          <Link href="/loginQ/login">
            <button className="px-6 py-2 bg-teal-600 text-white rounded-lg shadow-lg hover:bg-teal-700 transition-transform transform hover:scale-105">
              {t('login')}
            </button>
          </Link>
        </div>
        ...
        <section id="resources" className="pt-16 -mt-16 mb-16 px-6 text-center">
          <h2 className="text-3xl font-semibold text-teal-600 drop-shadow-lg">{t('resourses')}</h2>
          <p className="text-gray-700 mt-4 max-w-xl mx-auto">{t('descriptionresourses')}</p>
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {['articles', 'videos', 'podcast', 'resoursesin', 'communities', 'recommendations'].map((resource, index) => (
              <div key={index} className="bg-white rounded-lg shadow-lg p-4 transition-transform transform hover:scale-105">
                <h3 className="text-xl font-bold text-green-600">{t(resource)}</h3>
                <p className="text-gray-600 mt-2">{t(`description${resource}`)}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="ai" className="pt-16 -mt-16 mb-16 px-6 text-center">
          <h2 className="text-3xl font-semibold text-teal-600 drop-shadow-lg">{t('ia')}</h2>
          <p className="text-gray-700 mt-4 max-w-xl mx-auto">{t('descriptionia')}</p>
        </section>
        ...
        <section id="impact" className="pt-16 -mt-16 mb-16 px-6">
          <div className="bg-gradient-to-r from-green-50 to-teal-50 py-8 px-6 rounded-lg shadow-lg text-center max-w-2xl mx-auto">
            <h2 className="text-3xl font-semibold text-teal-600 drop-shadow-lg">{t('odss')}</h2>
            <p className="text-gray-700 mt-4">{t('descriptionods')}</p>
          </div>
        </section>
